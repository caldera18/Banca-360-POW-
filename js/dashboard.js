...
function formatCurrency(amount) { 
  return new Intl.NumberFormat("es-VE", { style: "currency", currency: "VES", minimumFractionDigits: 2 }).format(amount).replace("VES", "Bs."); 
}
...
function renderTransactions() {
  ...
  if (!filtered.length) { 
    container.innerHTML = '<p style="text-align: center; padding: 20px; color: var(--muted);">No hay movimientos para este filtro</p>'; 
    return; 
  }
...
function setupBalanceToggle() { 
  ...
  b.addEventListener("click", () => { 
    ...
    b.innerHTML = isVisible ? '<i class="fa-regular fa-eye"></i>' : '<i class="fa-regular fa-eye-slash"></i>'; 
  }); 
}
